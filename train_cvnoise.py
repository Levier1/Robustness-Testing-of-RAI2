...
import utils
import utils_noise  # 导入新的 utils_noise 模块
...
if __name__ == '__main__':
    ...
    if args.subset is None:
        set_mean, set_std = utils_noise.get_dataset_mean_std(args.dataset)
        training_loader = utils_noise.get_training_dataloader(args.dataset, set_mean, set_std, num_workers=8, batch_size=args.b, shuffle=True)
        chkfolder_subset = f"_{args.subset}"
    else:
        training_loader, set_mean, set_std = utils_noise.get_intersect_dataloader(dataset=args.dataset, propor=args.inter_propor, num_workers=8, batch_size=args.b, shuffle=True, sub_idx=args.subset-1)
        chkfolder_subset = f"subset{args.subset}_{args.dataset}_intersect_{args.inter_propor}"

    test_loader = utils_noise.get_test_dataloader(args.dataset, set_mean, set_std, num_workers=8, batch_size=args.b, shuffle=False)

    if args.rand_seed is not None:
        chkfolder_subset += "_rs{}".format(args.rand_seed)

    # process training hyperparameters
    epochs, milestones = utils_noise.get_dataset_hyperparam(args.dataset)
    ...
    checkpoint_path = os.path.join(settings.CHECKPOINT_PATH_NOISE, args.net + chkfolder_subset)
    ...
    results_file_path = '/home/featurize/work/RAI2project/RAI2checkpoint/noise_similarity/noise_cifar100_best_acc_results.txt'
