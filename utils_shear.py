...
# 这里只是把utils里关于数据集加载以及一些设置用到的函数单独拿出来并修改 例如加载和存储路径 其它的一些框架调用还是沿用utils里面的函数
...
def get_intersection_mean_std_dict(dataset_name):
    ...
    for s in (np.arange(11) / 10):
        Set1, Set2 = pickle.load(
            open(os.path.join(settings.DATA_PATH_NOISE, f'shear/{dataset_name.upper()}_intersect_{s}.pkl'), 'rb'))
...
def get_intersection_mean_std(dataset_name, inter_propor):
    ...
    # 构建文件路径，使用传入的比例值
    file_path = os.path.join(settings.DATA_PATH_SHEAR, f'shear/{dataset_name.upper()}_intersect_{inter_propor}.pkl')
...
def get_subtraining_dataloader_cifar10_intersect(propor=0.5, batch_size=16, num_workers=8, shuffle=True, sub_idx=1):
    X_set, y_set = \
    pickle.load(open(os.path.join(settings.DATA_PATH_SHEAR, f'shear/CIFAR10_intersect_{propor}.pkl'), 'rb'))[sub_idx]
...
def get_subtraining_dataloader_cifar100_intersect(propor=0.5, batch_size=16, num_workers=8, shuffle=True, sub_idx=1):
    X_set, y_set = \
    pickle.load(open(os.path.join(settings.DATA_PATH_SHEAR, f'shear/CIFAR100_intersect_{propor}.pkl'), 'rb'))[sub_idx]
    mean = tuple((X_set / 255).mean(axis=(0, 1, 2)))
    std = tuple((X_set / 255).std(axis=(0, 1, 2)))
...
def get_subtraining_dataloader_tinyimagenet_intersect(propor=0.5, batch_size=16, num_workers=8, shuffle=True,
                                                      sub_idx=1):
    X_set, y_set = \
    pickle.load(open(os.path.join(settings.DATA_PATH_SHEAR, f'shear/TINYIMAGENET_intersect_{propor}.pkl'), 'rb'))[
        sub_idx]
    mean = tuple((X_set / 255).mean(axis=(0, 1, 2)))
    std = tuple((X_set / 255).std(axis=(0, 1, 2)))
